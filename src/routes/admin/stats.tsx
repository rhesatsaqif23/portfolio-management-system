...
function StatsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) => createStat({ data: payload }),
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: { id: string; data: Record<string, unknown> }) => updateStat({ data: payload }),
  ...
  function openCreate() {
    const nextSort = stats.length > 0 ? Math.max(...stats.map((s) => s.sortOrder ?? 0)) + 1 : 0
  ...
  function executeConfirm() {
    ...
    if (confirm.type === 'create') {
      const { sortOrder, ...createPayload } = form
  ...
  return (
    <div>
      <div className="mb-4 md:mb-6 flex items-center justify-between">
        ...
        <Button size="sm" onClick={openCreate}><Plus className="size-4 md:size-5" /><span className="md:inline"> Create Stat</span></Button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/50">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl md:rounded-2xl border bg-card p-4 md:p-6 shadow-lg">
            ...
            <h2 className="text-sm md:text-lg font-semibold">{editing ? 'Edit Stat' : 'Create Stat'}</h2>
            <Button type="button" size="xs" variant="ghost" onClick={closeForm} className="text-muted-foreground">✕</Button>
          </div>
            <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
              <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                <TextField label="Key" name="key" value={form.key} onChange={(v) => setForm({ ...form, key: v })} error={errors.key} placeholder="e.g. years_experience" required />
                <TextField label="Category" name="category" value={form.category} onChange={(v) => setForm({ ...form, category: v })} placeholder="e.g. General" />
              </div>
              <TextField label="Value" name="value" value={form.value} onChange={(v) => setForm({ ...form, value: v })} error={errors.value} placeholder="e.g. 5+" required />
              <TextField label="Sub Value" name="subValue" value={form.subValue} onChange={(v) => setForm({ ...form, subValue: v })} placeholder="e.g. years" />
              <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                <TextField label="Icon" name="icon" value={form.icon} onChange={(v) => setForm({ ...form, icon: v })} placeholder="e.g. 🚀" />
      ...
      <AlertDialog open={!!confirm} onOpenChange={(o) => { if (!o) setConfirm(null) }}>
        <AlertDialogContent size="sm">
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>{confirm?.type === 'delete' ? 'Delete Stat' : confirm?.type === 'create' ? 'Create Stat' : 'Update Stat'}</AlertDialogTitle>
            <AlertDialogDescription>
              {confirm?.type === 'delete' ? 'This action cannot be undone. Are you sure?' : `Are you sure you want to ${confirm?.type === 'create' ? 'create' : 'update'} this stat?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
