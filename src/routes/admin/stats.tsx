...
function StatsPage() {
  ...
  function openCreate() {
    const nextSort =
      stats.length > 0
        ? Math.max(...stats.map((s) => s.sortOrder ?? 0)) + 1
        : 0;
    setEditing(null);
  ...
  function executeConfirm() {
    ...
    if (confirm.type === "create") {
      const { sortOrder: _sortOrder, ...createPayload } = form;
  ...
  return (
    <div>
      <div className="mb-4 md:mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-lg md:text-2xl font-bold text-(--sea-ink)">
            Stats
          </h1>
          <p className="mt-1 text-xs md:text-sm text-(--sea-ink-soft)">
            Manage About section stats.
          </p>
        </div>
        <Button size="sm" onClick={openCreate}>
          <Plus className="size-4 md:size-5" />
          <span className="md:inline"> Create Stat</span>
        </Button>
      </div>
      ...
      <div className="overflow-x-auto">
        <DataTable
          ...
          columns={[
            ...
            {
              key: "value" as keyof Stat,
              header: "Value",
              render: (v) => <span className="line-clamp-2">{String(v)}</span>,
            },
            { key: "category" as keyof Stat, header: "Category" },
            {
              key: "subValue" as keyof Stat,
              header: "Sub Value",
              render: (v) => (
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirm?.type === "delete"
                ...
                : `Are you sure you want to ${confirm?.type === "create" ? "create" : "update"} this stat?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
